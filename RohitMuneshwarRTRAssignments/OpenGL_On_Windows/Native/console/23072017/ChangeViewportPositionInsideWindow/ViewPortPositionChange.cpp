...
void resize(int width,int height){
	...
	if(width <= height){
		...
		//this requires from mobile screen
		...
		top = top * (GLfloat)width/(GLfloat)height;
		bottom = bottom * (GLfloat)width/(GLfloat)height;
	}
	glOrtho(left,right,bottom,top,-50.0f,50.0f);
}
