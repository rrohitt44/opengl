...
GLboolean gbAngleX = GL_TRUE;
GLboolean gbAngleY = GL_FALSE;
GLboolean gbAngleZ = GL_FALSE;
...
void display(){
	...
	if(gbAngleX==GL_TRUE){
		...
		light_position[0] = angleRedLight;
	}else if(gbAngleY==GL_TRUE){
		//glRotatef(angleRedLight,0,1,0);
		light_position[1] = angleGreenLight;
	}else if(gbAngleZ==GL_TRUE){
		//glRotatef(angleRedLight,0,0,1);
		light_position[2] = angleBlueLight;
	}
	glLightfv(GL_LIGHT0,GL_POSITION,light_position);
