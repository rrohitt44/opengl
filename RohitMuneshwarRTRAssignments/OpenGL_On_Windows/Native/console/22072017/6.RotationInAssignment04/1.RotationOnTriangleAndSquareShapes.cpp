...
//for Rotation
float gfAngleTri = 0.0f;
float gfAngleRect = 0.0f;
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLIne, int iCmdShow){
	void initialize(void);
	void uninitialize(void);
	void display(void);//double buffer sathi
	void updateAngle(void); //rotation
	WNDCLASSEX wndclass;
	TCHAR AppName[] = TEXT("Window Custom");
	TCHAR WinName[] = TEXT("Geometry - Rotating Shapes");
	HWND hwnd;
	MSG msg;
	RECT rect;
...
void display(){
	...
	glLoadIdentity();
	glTranslatef(1.5f,0.0f,-6.0f);
	glRotatef(gfAngleRect,1.0f,0.0f,0.0f);
	DrawRectangle();
...
void updateAngle(){
	...
	gfAngleRect = gfAngleRect - 0.1f;
	if(gfAngleRect<=-360.0f){
		gfAngleRect = 0.0f;
	}
