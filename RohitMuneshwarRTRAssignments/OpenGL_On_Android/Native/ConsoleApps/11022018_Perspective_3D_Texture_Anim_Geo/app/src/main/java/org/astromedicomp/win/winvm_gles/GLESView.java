...
import org.astromedicomp.win.persp_3d_texture_anim_geo.R;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private void initialize(GL10 gl)
	{
		...
		//triangle color
		GLES32.glGenBuffers(1, gVbo_pyramid_texture, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_pyramid_texture[0]);
		
		ByteBuffer triColByteBuffer = ByteBuffer.allocateDirect(pyramidTexcoords.length * 4);
		triColByteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer triColVerticesBuffer = triColByteBuffer.asFloatBuffer();
		triColVerticesBuffer.put(pyramidTexcoords);
		triColVerticesBuffer.position(0);
		
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramidTexcoords.length * 4, triColVerticesBuffer, GLES32.GL_STATIC_DRAW);
		...
		ByteBuffer squareColByteBuffer = ByteBuffer.allocateDirect(cubeTexcoords.length * 4);
		squareColByteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer squareColorBuffer = squareColByteBuffer.asFloatBuffer();
		squareColorBuffer.put(cubeTexcoords);
		squareColorBuffer.position(0);
		
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, cubeTexcoords.length * 4, squareColorBuffer, GLES32.GL_STATIC_DRAW);
		GLES32.glVertexAttribPointer(GLESMacros.SAM_ATTRIBUTE_TEXTURE0,
                                     2,
                                     GLES32.GL_FLOAT,
                                     false,0,0);
	...
	void uninitialize()
    {
		...
		// destroy texture vbo
        if(gVbo_pyramid_texture[0] != 0)
        {
            GLES32.glDeleteBuffers(1, gVbo_pyramid_texture, 0);
            gVbo_pyramid_texture[0]=0;
        }
		...
		// destroy texture vbo
        if(gVbo_Cube_texture[0] != 0)
        {
            GLES32.glDeleteBuffers(1, gVbo_Cube_texture, 0);
            gVbo_Cube_texture[0]=0;
        }
	...
	gAnglePyramid=gAnglePyramid+0.75f;
	gAngleCube=gAngleCube+0.75f;
}
}