...
void initialize() {
	...
	LoadGLTexture("minions.bmp", &Texture_Dada);
	LoadGLTexture("StoneWall.png", &Texture_StoneWall);
...
void display(void) {
	...
	void drawColoredPyramid(void);
	void drawColoredCube(void);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	//glClear(GL_COLOR_BUFFER_BIT );
	glLoadIdentity();

	glTranslatef(-1.5f, 0.0f, (GLfloat) -6.0);
	glRotatef(rotateTriangleAngle, 0.0f, 1.0f, 0.0f);
	glBindTexture(GL_TEXTURE_2D, Texture_StoneWall);
	drawColoredPyramid();

	glLoadIdentity();
	glTranslatef(1.5f, 0.0f, (GLfloat) -6.0);
	glRotatef(rotateSquareAngle, 0.0f, 1.0f, 0.0f);
	glBindTexture(GL_TEXTURE_2D, Texture_Dada);
	drawColoredCube();
	//glFlush();
...
void rotate() {
	...
	rotateTriangleAngle = rotateTriangleAngle + 0.01f;

	if (rotateSquareAngle == 0.0f)
		rotateSquareAngle = 360.0f;
	rotateSquareAngle = rotateSquareAngle - 0.01f;
}
...
void uninitialize(void) {
	...
	if (Texture_Dada) {
		glDeleteTextures(1, &Texture_Dada);
		Texture_Dada = 0;
	}

	if (Texture_StoneWall) {
		glDeleteTextures(1, &Texture_StoneWall);
		Texture_StoneWall = 0;
	}
